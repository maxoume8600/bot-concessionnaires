...
async function handlePrendreServiceButton(interaction) {
    ...
    // Détection automatique du poste selon les rôles
    let poste = '🏢 Standard';
    
    if (interaction.member?.roles?.cache) {
        if (interaction.member.roles.cache.some(role => role.name.toLowerCase().includes('patron'))) {
            poste = '👑 Patron';
        } else if (interaction.member.roles.cache.some(role => role.name.toLowerCase().includes('co-patron'))) {
            poste = '👑 Co-Patron';
        } else if (interaction.member.roles.cache.some(role => role.name.toLowerCase().includes('directeur'))) {
            poste = '💼 Directeur';
        } else if (interaction.member.roles.cache.some(role => role.name.toLowerCase().includes('manager') || role.name.toLowerCase().includes('gérant'))) {
            poste = '👔 Manager';
        } else if (interaction.member.roles.cache.some(role => role.name.toLowerCase().includes('vendeur') || role.name.toLowerCase().includes('commercial'))) {
            poste = '🤝 Vendeur';
        } else if (interaction.member.roles.cache.some(role => role.name.toLowerCase().includes('mécanicien') || role.name.toLowerCase().includes('technicien'))) {
            poste = '🔧 Mécanicien';
        } else if (interaction.member.roles.cache.some(role => role.name.toLowerCase().includes('stagiaire') || role.name.toLowerCase().includes('apprenti'))) {
            poste = '📚 Stagiaire';
        }
    }
